...
def AttentionLayer(inputs,ks = 1,out_cls = 7,decay_weight=-1.):
    ...
    attention = layers.Conv2D(filters=out_cls,kernel_size=ks,strides=1,padding='same',use_bias=False,activation='softmax')(inputs)
...
def MakeConvModel(name="RepVgg"):
    ...
    s1 = AttentionLayer(s1,ks=5,decay_weight=-1.)
...
modelRecordPath = r'/home/featurize/data/SavedModel1'
decay_steps = 200
...
model.fit(train_dataset,validation_data=test_dataset,epochs=200,callbacks=[lr_callback,checkPointCallBack])
#%%
